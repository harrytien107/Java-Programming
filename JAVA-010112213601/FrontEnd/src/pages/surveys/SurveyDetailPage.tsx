        ...
        <Container maxWidth="md" sx={{ py: 4 }}>
          <Paper sx={{ p: 4, textAlign: "center" }}>
            ...
            <Typography variant="body1" color="text.secondary" paragraph>
              Cảm ơn bạn đã tham gia khảo sát "{survey.name}". Phản hồi của bạn rất quan trọng đối với chúng tôi.
            </Typography>
            ...
            <Box sx={{ my: 3, p: 3, bgcolor: "primary.light", borderRadius: 2 }}>
              ...
              <Typography variant="body2" sx={{ color: "primary.contrastText", mt: 1 }}>
                ...
                /{survey.questions.length} câu đúng)
              </Typography>
            </Box>

            <Box sx={{ mt: 3 }}>
              <Button component={Link} to="/surveys" variant="contained" sx={{ mr: 2 }}>
                Khảo sát khác
              </Button>
      ...
      <Container maxWidth="md" sx={{ py: 4 }}>
        ...
        <Box sx={{ mb: 4 }}>
          <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 2 }}>
            <Button component={Link} to="/surveys" startIcon={<ArrowBackIcon />}>
              Quay lại
            </Button>
            <Button onClick={loadMarkHistory} variant="outlined" size="small">
              Lịch sử của bài này
            </Button>
          </Box>
          <Typography variant="h4" gutterBottom>
        ...
        <Card sx={{ mb: 4 }}>
          <CardContent sx={{ p: 4 }}>
            <FormControl component="fieldset" fullWidth>
              ...
              <RadioGroup value={currentQuestion.id ? answers[currentQuestion.id] : -1} onChange={(e) => currentQuestion.id && handleAnswerChange(currentQuestion.id, parseInt(e.target.value))}>
        ...
        <Box sx={{ display: "flex", justifyContent: "space-between" }}>
          <Button onClick={handleBack} disabled={activeStep === 0} startIcon={<ArrowBackIcon />} variant="outlined">
            Câu trước
          </Button>

          {activeStep === survey.questions.length - 1 ? (
            <Button onClick={handleSubmit} disabled={!isCurrentQuestionAnswered()} variant="contained" endIcon={<CheckCircleIcon />}>
              Hoàn thành
            </Button>
          ) : (
            <Button onClick={handleNext} disabled={!isCurrentQuestionAnswered()} variant="contained" endIcon={<ArrowForwardIcon />}>
              Câu tiếp theo
            </Button>
          )}
        </Box>
        ...
        <Dialog open={openDialog} onClose={() => setOpenDialog(false)}>
          ...
          <DialogContent>
            <Typography>Bạn có chắc chắn muốn nộp bài khảo sát này không? Sau khi nộp bài, bạn sẽ không thể thay đổi câu trả lời.</Typography>
          </DialogContent>
          <DialogActions>

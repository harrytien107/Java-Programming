...
function TabPanel(props: TabPanelProps) {
  ...
  return (
    <div role="tabpanel" hidden={value !== index} id={`appointment-tabpanel-${index}`} aria-labelledby={`appointment-tab-${index}`} {...other}>
...
const AppointmentsPage: React.FC<AppointmentsPageProps> = ({ isAdmin = false }) => {
  ...
  const upcomingAppointments = appointments.filter((appointment) => (appointment.status === "PENDING" || appointment.status === "CONFIRM") && new Date(appointment.date) >= new Date());

  const pastAppointments = appointments.filter((appointment) => appointment.status === "COMPLETE" || appointment.status === "CANCEL" || new Date(appointment.date) < new Date());
    ...
    <Container>
                ...
                return (
                  <React.Fragment key={appointment.id}>
                    <ListItem alignItems="flex-start">
                      <ListItemAvatar>
                        <Avatar src={getSpecialistAvatar(appointment.specialistFullname)} alt={appointment.specialistFullname}>
                          <PersonIcon />
                        </Avatar>
                      </ListItemAvatar>
                      <ListItemText
                        primary={
                          <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                            ...
                            <Chip label={getStatusText(appointment.status)} color={getStatusColor(appointment.status)} size="small" />
                          </Box>
                        }
                        secondary={
                          <React.Fragment>
                            ...
                            <Box sx={{ mt: 2 }}>
                              <Button variant="outlined" size="small" sx={{ mr: 1 }} disabled>
                                Đổi lịch
                              </Button>
                              {appointment.status === "PENDING" && (
                                <Button variant="outlined" color="error" size="small" onClick={() => handleChangeStatus(appointment.id, "CANCEL")}>
                                  Hủy lịch
                                </Button>
                              )}
                              {appointment.status !== "PENDING" && (
        ...
        <TabPanel value={tabValue} index={1}>
          {pastAppointments.length > 0 ? (
            ...
            <Box sx={{ textAlign: "center", py: 4 }}>
              ...
              <Typography variant="body1">Lịch sử cuộc hẹn sẽ hiển thị ở đây sau khi bạn hoàn thành các cuộc hẹn.</Typography>
            </Box>
          )}
        </TabPanel>
      </Paper>
      ...
      <Dialog open={openDialog} onClose={handleCloseDialog} maxWidth="md" fullWidth>
              ...
              <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "1fr 1fr" }, gap: 3 }}>
                <Box>
                  <FormControl fullWidth sx={{ mb: 3 }}>
                    ...
                    <Select labelId="specialist-select-label" id="specialist-select" value={selectedSpecialist} label="Chuyên gia tư vấn" onChange={(e) => setSelectedSpecialist(e.target.value)}>
                  ...
                  <TextField label="Giờ hẹn" type="time" value={selectedTime} onChange={(e) => setSelectedTime(e.target.value)} fullWidth InputLabelProps={{ shrink: true }} sx={{ mb: 3 }} />
                </Box>

                <Box>
                  <TextField label="Thời lượng (phút)" type="number" value={duration} onChange={(e) => setDuration(parseFloat(e.target.value))} fullWidth sx={{ mb: 3 }} InputProps={{ inputProps: { min: 1, step: 15 } }} helperText="Thời lượng phải lớn hơn 0 phút" />

                  <Alert severity="info" sx={{ mb: 3 }}>
                    <Typography variant="body2">Vui lòng đặt lịch hẹn trước ít nhất 24 giờ để đảm bảo chuyên gia tư vấn có thể sắp xếp thời gian phù hợp.</Typography>
                  </Alert>

                  <Alert severity="warning">
                    <Typography variant="body2">Nếu bạn cần hủy hoặc đổi lịch, vui lòng thông báo trước ít nhất 12 giờ.</Typography>
                  </Alert>
                </Box>
              </Box>
            </Box>
        ...
        <DialogActions>
          ...
          <Button variant="contained" onClick={handleBookAppointment} disabled={!selectedSpecialist || !selectedDate || !selectedTime || duration <= 0 || bookingSuccess || loading}>

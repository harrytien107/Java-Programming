    ...
    try {
      if (dialogMode === "add") {
        ...
        if (code === 200) {
          ...
          toast.info("Đang cập nhật danh sách...", { autoClose: 1000 });
          setTimeout(() => {
            loadCourses();
          }, 500);
        } else {
        ...
        if (response.code === 200) {
          ...
          // Show update indicator and refresh
          toast.info("Đang cập nhật danh sách...", { autoClose: 1000 });
          setTimeout(() => {
            loadCourses();
          }, 500);
  ...
  return (
    <Container>
      ...
      <Paper sx={{ p: 2, mb: 3 }}>
        <Box sx={{ display: "flex", flexWrap: "wrap", gap: 2, alignItems: "center" }}>
          ...
          <FormControl variant="outlined" size="small" sx={{ minWidth: "150px" }}>
            ...
            <Select labelId="object-filter-label" id="object-filter" value={objectFilter} onChange={handleObjectFilterChange} label="Đối tượng">
      ...
      <Paper sx={{ width: "100%", overflow: "hidden" }}>
                    ...
                    <TableCell>
                      <Box sx={{ display: "flex", flexWrap: "wrap", gap: 0.5 }}>
                        {course.objects.map((object) => (
                          <Chip key={object} label={object} size="small" color="primary" variant="outlined" />
                        ))}
                      </Box>
                    </TableCell>
                    <TableCell>{new Date(course.createDate).toLocaleDateString("vi-VN")}</TableCell>
                    <TableCell align="center">
                      <Tooltip title="Chỉnh sửa">
                        <IconButton color="primary" onClick={() => handleOpenEditDialog(course)}>
                          <EditIcon />
                        </IconButton>
                      </Tooltip>
                      <Tooltip title="Xóa">
        ...
        <TablePagination rowsPerPageOptions={[5, 10, 25]} component="div" count={totalElements} rowsPerPage={rowsPerPage} page={page} onPageChange={handleChangePage} onRowsPerPageChange={handleChangeRowsPerPage} labelRowsPerPage="Số hàng mỗi trang:" labelDisplayedRows={({ from, to, count }) => `${from}-${to} của ${count}`} />
      </Paper>

      {/* Add/Edit Course Dialog */}
      <Dialog open={openCourseDialog} onClose={handleCloseCourseDialog} maxWidth="lg" fullWidth>
        <DialogTitle>{dialogMode === "add" ? "Thêm khóa học mới" : "Chỉnh sửa khóa học"}</DialogTitle>
        <DialogContent>
            ...
            <Card>
              <CardContent>
                ...
                <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
                  <TextField label="Tên khóa học" fullWidth value={formData.name} onChange={(e) => handleFormChange("name", e.target.value)} required />
                  <TextField label="Mô tả" fullWidth multiline rows={3} value={formData.description} onChange={(e) => handleFormChange("description", e.target.value)} required />
                  ...
                  <Box>
                    ...
                    <FormGroup row>
                      {OBJECTS.map((object) => (
                        <FormControlLabel key={object} control={<Checkbox checked={formData.objects.includes(object)} onChange={() => handleObjectChange(object)} />} label={object} />
                      ))}
                    </FormGroup>
                  </Box>
                </Box>
            ...
            <Card>
              <CardContent>
                <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 2 }}>
                  ...
                  <Button variant="outlined" startIcon={<AddIcon />} onClick={addCourseDetail}>
                    Thêm bài học
                  </Button>
                </Box>
                  ...
                  <Card key={index} variant="outlined" sx={{ mb: 2 }}>
                    <CardContent>
                      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 2 }}>
                        ...
                        <IconButton color="error" onClick={() => removeCourseDetail(index)}>
                          <DeleteIcon />
                        </IconButton>
                      </Box>

                      <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
                        <TextField label="Tên bài học" fullWidth value={detail.name} onChange={(e) => handleCourseDetailChange(index, "name", e.target.value)} required />

                        <Box sx={{ display: "flex", gap: 2, alignItems: "flex-start" }}>
                          <TextField label="Thời lượng (giờ)" type="number" value={detail.duration} onChange={(e) => handleCourseDetailChange(index, "duration", parseFloat(e.target.value) || 1)} inputProps={{ min: 0.1, step: 0.1 }} sx={{ width: "200px" }} required />

                          <Box sx={{ flexGrow: 1 }}>
                            <Button variant="outlined" component="label" startIcon={<VideoIcon />} fullWidth>
                              Chọn video
                              <input type="file" hidden accept="video/*" onChange={(e) => handleVideoUpload(index, e)} />
                            </Button>
                            {videoFiles[index] && (
                ...
                {formData.courseDetail.length === 0 && <Alert severity="info">Chưa có bài học nào. Nhấn "Thêm bài học" để bắt đầu.</Alert>}
              </CardContent>
            </Card>
          </Box>
        </DialogContent>
        <DialogActions>
          ...
          <Button onClick={handleSaveCourse} variant="contained" disabled={loading || !formData.name || !formData.description}>
      ...
      <Dialog open={openDeleteDialog} onClose={handleCloseDeleteDialog}>
        ...
        <DialogContent>
          <Typography>Bạn có chắc chắn muốn xóa khóa học "{courseToDelete?.name}"? Hành động này không thể hoàn tác.</Typography>
        </DialogContent>
        <DialogActions>
          <Button onClick={handleCloseDeleteDialog}>Hủy</Button>
          <Button onClick={handleDeleteCourse} variant="contained" color="error" disabled={loading}>

...
import { Container, Typography, Box, Card, CardContent, CardMedia, Button, Alert } from "@mui/material";
import { CalendarMonth as CalendarMonthIcon, LocationOn as LocationOnIcon, People as PeopleIcon, ArrowBack as ArrowBackIcon } from "@mui/icons-material";
...
const RegisteredProgramsPage: React.FC = () => {
  ...
  return (
    <ClientLayout>
      <Container>
        <Box sx={{ mb: 3 }}>
          <Button component={Link} to="/programs" startIcon={<ArrowBackIcon />} sx={{ mb: 2 }}>
        ...
        {loading ? (
          <Box sx={{ textAlign: "center", py: 4 }}>
            <Typography variant="h6">Đang tải danh sách chương trình...</Typography>
          </Box>
          ...
          <Box sx={{ textAlign: "center", py: 4 }}>
            ...
            <Button component={Link} to="/programs" variant="contained" color="primary">
            ...
            {programs.map((program) => (
              <Card key={program.id} sx={{ borderRadius: 2, overflow: "hidden" }}>
                ...
                <CardContent>
                  ...
                  <Button component={Link} to={`/programs/${program.id}`} variant="outlined" fullWidth>
                    Xem chi tiết
                  </Button>
                </CardContent>
              </Card>
